...
func (interactor *OrderInteractor) Add(userId, orderId, itemId int) error {
	...
	interactor.Logger.Log(fmt.Sprintf("User added item '%s' (#%i) to order #%i", item.Name, item.Id, order.Id))
	return nil
}
...
func (interactor *AdminOrderInteractor) Add(userId, orderId, itemId int) error {
	...
	order := interactor.OrderRepository.FindById(orderId)
	if !user.IsAdmin {
		message = "User #%i (customer #%i) is not allowed to add items to order #%i (of customer #%i), because they are not an administrator"
		err := fmt.Errorf(message, user.Id, user.Customer.Id, order.Id, order.Customer.Id)
		interactor.Logger.Log(err.Error())
		return err
	}
	...
	interactor.OrderRepository.Store(order)
	interactor.Logger.Log(fmt.Sprintf("Admin added item '%s' (#%i) to order #%i", item.Name, item.Id, order.Id))
	return nil
}
