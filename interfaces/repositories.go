...
import (
	"fmt"
	"github.com/RiverPhillips/clean-architecture-go/domain"
...
func (repo *DbUserRepo) Store(user usecases.User) {
	...
	repo.dbHandler.Execute(fmt.Sprintf(`INSERT into users (id, customer_id, is_admin) VALIES ('%d', '%d', '%v')`,
...
func (repo *DbUserRepo) FindById(id int) usecases.User {
	row := repo.dbHandler.Query(fmt.Sprintf("SELECT is_admin, customer_id FROM users WHERE id ='%d' LIMIT 1",
		id))
...
func (repo *DbCustomerRepo) Store(customer domain.Customer) {
	repo.dbHandler.Execute(fmt.Sprintf(`INSERT INTO customers (id, name) VALUES ('%d', '%v')`,
		customer.Id, customer.Name))
}

func (repo *DbCustomerRepo) FindById(id int) domain.Customer {
	row := repo.dbHandler.Query(fmt.Sprintf(`SELECT name FROM customers WHERE id = '%d' LIMIT 1`,
		id))
...
func NewDbOrderRepo(dbHandlers map[string]DbHandler) *DbOrderRepo {
	dbCustomerRepo := new(DbOrderRepo)
	dbCustomerRepo.dbHandlers = dbHandlers
	dbCustomerRepo.dbHandler = dbHandlers["DbOrderRepo"]
	return dbCustomerRepo
}

func (repo *DbOrderRepo) Store(order domain.Order) {
	repo.dbHandler.Execute(fmt.Sprintf(`INSERT INTO orders (id, customer_id) VALUES ('%d', '%v')`,
		order.Id, order.Customer.Id))
	for _, item := range order.Items {
		repo.dbHandler.Execute(fmt.Sprintf(`INSERT into item2orders (item_id, order_id) VALUES ('%d', '%v')`,
			item.Id, order.Id))
	}
}

func (repo *DbOrderRepo) FindById(id int) domain.Order {
	row := repo.dbHandler.Query(fmt.Sprintf(`SELECT customer_id FROM orders WHERE id = '%d' LIMIT 1`,
		id))
	...
	row = repo.dbHandler.Query(fmt.Sprintf(`SELECT item_id FROM item2orders WHERE order_id = '%d'`,
		order.Id))
...
func (repo *DbItemRepo) Store(item domain.Item) {
	...
	repo.dbHandler.Execute(fmt.Sprintf(`INSERT INTO items (id, name, value, available) VALUES ('%d', '%v', '%f', '%v')`,
		item.Id, item.Name, item.Value, available))
}

func (repo *DbItemRepo) FindById(id int) domain.Item {
	row := repo.dbHandler.Query(fmt.Sprintf(`SELECT name, value, available FROM items WHERE id = '%d' LIMIT 1`,
		id))
